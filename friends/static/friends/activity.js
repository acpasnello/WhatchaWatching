...
function activityBlock(activity, displayDiv) {
    ...
    if (activity.action == "reviewed") {
        ...
        newDiv.innerHTML = 
        `<div>
            <a href="/profile/${activity.userID}" class="friendsresult">${activity.user}</a> ${activity.action} <a href="${activity.medialink}" class="browseList">${activity.listitem}</a> to the list, <a href="${activity.listlink}">${activity.list}</a>.
        </div>`
    }
